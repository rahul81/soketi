...
export class HttpHandler {
    ...
    acceptTraffic(res: HttpResponse) {
        this.attachMiddleware(res, [this.corsMiddleware]).then((res) => {
            ...
            let threshold =
                this.server.options.httpApi.acceptTraffic.memoryThreshold;

            let { rss, heapTotal, external, arrayBuffers } =
                process.memoryUsage();
        ...
        this.attachMiddleware(res, [
            ...
            if (!res.params.channel.startsWith("presence-")) {
                return this.badResponse(
                    res,
                    "The channel must be a presence channel."
                );
            }
    ...
    events(res: HttpResponse) {
        this.attachMiddleware(res, [
            ...
            this.checkMessageToBroadcast(
                res.body as PusherApiMessage,
                res.app as App
            )
                .then((message) => {
    ...
    protected checkMessageToBroadcast(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (
                payloadSizeInKb > parseFloat(app.maxEventPayloadInKb as string)
            ) {
                return reject({
        ...
        this.readJson(
            ...
            (body, rawBody) => {
                ...
                if (
                    //@ts-ignore
                    requestSizeInMb >
                    this.server.options.httpApi.requestLimitInMb
                ) {
    ...
    protected authMiddleware(res: HttpResponse, next: CallableFunction): any {
        this.signatureIsValid(res).then((valid) => {
            ...
            return this.unauthorizedResponse(
                res,
                "The secret authentication failed"
            );
        });
    }
