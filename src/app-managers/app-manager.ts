...
export class AppManager implements AppManagerInterface {
    ...
    findById(id: string): Promise<App | null> {
        ...
        return this.server.cacheManager
            .get(`app:${id}`)
            .then((appFromCache) => {
                if (appFromCache) {
     ...
     */
    async selectFlowsByAppId(id: string, channel: string): Promise<Flow[]> {
        console.log("OUT");
        let out = await this.driver.selectFlowsByAppId(id, channel);
        console.log(">>>> FLOW", out);
        return out;
    }

    async generateFlowKeys(): Promise<any[]> {
        let out = await this.driver.generateFlowKeys();
        console.log("OUT", out);
